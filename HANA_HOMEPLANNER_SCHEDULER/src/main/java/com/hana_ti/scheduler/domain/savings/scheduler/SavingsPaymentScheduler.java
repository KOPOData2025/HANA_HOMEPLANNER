...
@Slf4j
public class SavingsPaymentScheduler {

    private final SavingsPaymentService savingsPaymentService;

    /**
     * 매일 오전 9시에 적금 납입 처리 실행
     */
    @Scheduled(cron = "0 0 9 * * *")
    @Transactional
    public void processMonthlyPayments() {
        log.info("적금 매월 납입 스케줄러 시작 - 실행시간: {}", LocalDateTime.now());

        try {
            // 적금 납입 처리 실행
            SavingsPaymentService.SavingsPaymentResult result = savingsPaymentService.processMonthlyPayments();

            // 실행 결과 파일 저장 (데이터가 없어도 로그 생성)
            saveExecutionResult(result);

            log.info("적금 매월 납입 스케줄러 완료 - 성공: {}, 실패: {}, 오류: {}", 
                    result.getSuccessCount(), result.getFailureCount(), result.getErrorCount());

        } catch (Exception e) {
            log.error("적금 매월 납입 스케줄러 실행 중 오류 발생", e);
            saveErrorResult(e);
        }
    }
     ...
     * 실행 결과를 파일로 저장
     */
    private void saveExecutionResult(SavingsPaymentService.SavingsPaymentResult result) {
        LocalDate today = LocalDate.now();
        String fileName = String.format("/app/logs/savings_payment_result_%s.txt", today.format(DateTimeFormatter.ofPattern("yyyyMMdd")));
        
        try (FileWriter writer = new FileWriter(fileName, true)) {
            writer.write("=".repeat(80) + "\n");
            writer.write("적금 매월 납입 처리 결과\n");
    ...
    private void saveErrorResult(Exception e) {
        LocalDate today = LocalDate.now();
        String fileName = String.format("/app/logs/savings_payment_error_%s.txt", today.format(DateTimeFormatter.ofPattern("yyyyMMdd")));
        
        try (FileWriter writer = new FileWriter(fileName, true)) {
            writer.write("=".repeat(80) + "\n");
            writer.write("적금 매월 납입 처리 오류\n");
            writer.write("실행일시: " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")) + "\n");
            writer.write("=".repeat(80) + "\n\n");
